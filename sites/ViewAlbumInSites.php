...
function viewAlbumsInSites($Siteid, $jumpto)
...
xajax_DisplaySites('');\">Back&nbsp;&nbsp;</a></td>";
$objResponse->addAssign("option","innerHTML",$backlink);
	if($orderby =="")
	{$query=" SELECT *
FROM album, albumcontainsimages, imagetakeninsites
WHERE imagetakeninsites.ImageId = albumcontainsimages.ImageId
AND albumcontainsimages.AlbumId = album.AlbumId
AND imagetakeninsites.SiteId =$Siteid GROUP BY album.AlbumId
 ...
 limit $start, $rowsPerPage";
	...
	if($num==0)
	...
	else
		{
			...
			for($S=0;$S<$num;$S++)
				{	//get path of image
					...
					<a href=\"javascript:void(null);\"onclick=\"xajax_ShowGallery();xajax_createSiteAlbumView($albumId,'');xajax_RateAlbum($albumId);\" >";//end of echo
					//GetImage path 
		...
		$query="select  *  FROM album, albumcontainsimages, imagetakeninsites
WHERE imagetakeninsites.ImageId = albumcontainsimages.ImageId
AND albumcontainsimages.AlbumId = album.AlbumId
AND imagetakeninsites.SiteId =$Siteid GROUP BY album.AlbumId";
		...
		$result  = mysql_query($query) or die('Error, query failed'); 
				...
				if ($pageNum > 1) 
						{ 
							$page = $pageNum -1; 
							$prev="<input type=\"button\" name=\"prev\" onclick =\"xajax_viewAlbumsInSites($Siteid,$page);\" value=\"Prev\">"; 

							$first = "<input type=\"button\" name=\"first\" onclick =\"xajax_viewAlbumsInSites($Siteid,1);\" value=\"First\">";  
				...
				if ($pageNum < $maxPage) 
						{ 
						$page = $pageNum + 1; 
						$next = "<input type=\"button\" name=\"next\"onclick=\"xajax_viewAlbumsInSites($Siteid,$page);\"value=\"Next\">";  
						$last = "<input type=\"button\" name=\"Last\"onclick=\"xajax_viewAlbumsInSites($Siteid,$maxPage);\"value=\"Last\">";   
